 ...
 * Problema Risolto: Pagina pubblica di richiesta reset password (/password-dimenticata).
 * Invia l'email a POST /api/auth/forgot-password e mostra SEMPRE lo stesso
 * messaggio generico di conferma (coerente con l'anti-enumerazione del backend).
 */

import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { KeyRound, ChevronRight, ArrowLeft, MailCheck, AlertCircle } from 'lucide-react';
...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // Qualsiasi risposta del server (2xx/4xx/5xx) porta al medesimo messaggio
      // generico: non riveliamo se l'email è registrata.
      ...
      <div className="w-full max-w-md p-8 rounded-3xl glass-panel border border-app-border shadow-[0_30px_90px_rgba(0,0,0,0.8)] relative overflow-hidden transition-all duration-300">
        ...
        <div className="text-center mb-6 pt-2">
          <div className="w-14 h-14 bg-app-accent/15 border border-app-accent/30 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-inner">
            <KeyRound className="w-7 h-7 text-app-accent" />
          </div>
        ...
        {done ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {submitting ? 'Invio in corso...' : <>Invia link di reset <ChevronRight className="w-4 h-4" /></>}
            </button>
          </form>
        )}
